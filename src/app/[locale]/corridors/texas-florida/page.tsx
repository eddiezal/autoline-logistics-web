...
export default async function TexasFloridaCorridor({
  params,
}: {
  ...
  const corridor = getCorridor("texas-florida")!;
  const faqs: FAQItem[] = FAQ_INDEXES.map((i) => ({
    q: t(`corridors.texasFlorida.faq.items.${i}.q`),
    a: t(`corridors.texasFlorida.faq.items.${i}.a`),
  }));
  ...
  return (
    <>
      ...
      <Header />
      <CorridorAnalytics corridorSlug="texas-florida" fromState="TX" toState="FL" />
      ...
      <main className="flex-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10 lg:gap-16 items-start">
              <div>
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight">
                  {t("corridors.texasFlorida.hero.title")}
                </h1>
                <p className="text-lg md:text-xl text-gray-300 mt-5 max-w-xl leading-relaxed">
                  {t("corridors.texasFlorida.hero.description")}
                </p>

                <div className="flex flex-wrap gap-3 mt-8">
                  <Link
                    href={{ pathname: "/quote", query: { from: "TX", to: "FL" } }}
                  ...
                  >
                    {t("corridors.texasFlorida.hero.ctaPrimary")}
                  </Link>
                  <a
                    href="#math"
                    className="border border-white/30 hover:border-white text-white font-semibold px-6 py-3 rounded-full transition"
                  >
                    {t("corridors.texasFlorida.hero.ctaSecondary")}
                  </a>
                </div>
              </div>

              <CorridorPricingCard corridorSlug="texas-florida" locale={locale} />
            </div>
          </Container>
        </section>
          ...
          <Container>
            <div className="max-w-3xl">
              <p className="text-orange text-sm font-semibold uppercase tracking-wider mb-3">
                {t("corridors.texasFlorida.math.eyebrow")}
              </p>
              <h2 className="text-2xl md:text-3xl font-bold text-charcoal mb-6">
                {t("corridors.texasFlorida.math.title")}
              </h2>
              <div className="space-y-4 text-gray-700 leading-relaxed">
                <p>{t("corridors.texasFlorida.math.body.0")}</p>
                <p>{t("corridors.texasFlorida.math.body.1")}</p>
                <p className="font-semibold text-charcoal">
                  {t("corridors.texasFlorida.math.punchline")}
                </p>
              </div>
            </div>
          </Container>
        ...
        <section className="py-16 bg-gray-100">
          <Container>
            <p className="text-orange text-sm font-semibold uppercase tracking-wider mb-3">
              {t("corridors.texasFlorida.customers.eyebrow")}
            </p>
            <h2 className="text-2xl md:text-3xl font-bold text-charcoal max-w-2xl mb-8">
              {t("corridors.texasFlorida.customers.title")}
            </h2>
              ...
              {CUSTOMER_KEYS.map((k) => (
                ...
                >
                  <h3 className="text-lg font-bold text-charcoal mb-2">
                    {t(`corridors.texasFlorida.customers.items.${k}.title`)}
                  </h3>
                  <p className="text-gray-700 text-sm leading-relaxed">
                    {t(`corridors.texasFlorida.customers.items.${k}.body`)}
                  </p>
                </article>
              ))}
            </div>
        ...
        <section className="py-16 bg-gray-100">
          <Container>
            <div className="max-w-3xl">
              <p className="text-orange text-sm font-semibold uppercase tracking-wider mb-3">
                {t("corridors.texasFlorida.faq.eyebrow")}
              </p>
              <FAQ items={faqs} title={t("corridors.texasFlorida.faq.title")} />
            </div>
          </Container>
        </section>
          ...
          <Container>
            <div className="max-w-2xl mx-auto text-center">
              <h2 className="text-3xl md:text-4xl font-bold leading-tight mb-4">
                {t("corridors.texasFlorida.finalCta.title")}
              </h2>
              <p className="text-lg text-gray-300 mb-8">
                {t("corridors.texasFlorida.finalCta.body")}
              </p>
              <Link
                href={{ pathname: "/quote", query: { from: "TX", to: "FL" } }}
              ...
              >
                {t("corridors.texasFlorida.finalCta.cta")}
              </Link>
            </div>
          </Container>
        </section>
